# rescue_stt_node.py v0.110 2026-03-14
# [이번 버전에서 수정된 사항]
# - History 연동 이벤트 토픽에 reliable/transient_local/keep_last(1) QoS 적용
# - /robot6/tts/request 구독 QoS를 이벤트 퍼블리셔와 일치하도록 조정
...
class RescueDialogueNode(Node):
    def __init__(self):
        ...
        '기존 음성 기록 퍼블리셔'
        self.publisher_ = self.create_publisher(String, '/robot6/victim_voice_reply', event_qos)

        '행동 제어용 keep_going 토픽'
        self.keep_going_pub = self.create_publisher(String, '/robot6/keep_going', 10)

        'Control 노드 연동용 토픽'
        self.tts_done_pub = self.create_publisher(Bool, '/robot6/tts/done', event_qos)
