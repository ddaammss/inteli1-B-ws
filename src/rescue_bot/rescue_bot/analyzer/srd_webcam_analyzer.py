...
class SeverityAnalyzer:
    ...
    def _classify_observation(
        ...
        # [1. 시야/관측 범위 판별]
        # 인식된 유효 관절 개수를 바탕으로 대상이 화면에 어느 정도 들어와 있는지 파악합니다.
        # 하체(골반 등) 유효 관절 수에 따라 상반신(UPPER_BODY)과 전신(FULL_BODY)을 구분합니다.
    ...
    def _classify_posture(
        ...
        # [2.1 다중 포인트 앵커 적용]
        # 단일 관절(예: 코 한 곳)만 사용할 때 생기는 일시적 인식 오류를 방지하기 위해
        # 얼굴 5개 포인트(코, 양눈, 양귀)의 평균값(Anchor)을 기준으로 활용하여 안정성을 극대화합니다.
        ...
        # [2.3 상체(Torso) 각도 및 전신(FULL_BODY) 모드 판별]
        # 전신이 보일 경우 어깨-골반을 잇는 중심선의 각도를 구하여 대상이 바닥에 쓰러졌는지(LYING / COLLAPSED) 판단합니다.
        torso_angle = 0.0
    ...
    def _motion_value(
        ...
        # [3. 움직임(Motion) 활동량 평가]
        # 여러 프레임(기본 12프레임 버퍼) 간에 관절들이 픽셀 단위로 얼마나 이동했는지 누적 평균을 구합니다.
        # - upper: 손/팔 등 상체 관절들의 평균 이동량
        # - core: 어깨/골반 등 코어 관절들의 평균 이동량
        # - smooth: 버퍼에 저장되어 부드럽게 정규화된 전체 평균 이동량
    ...
    def _decide(
        ...
        # [4. 시간 추적을 통한 최종 시스템 위급도(Severity) 결정]
        # 단일 프레임에서의 오류나 찰나의 움직임 속임수를 방지합니다.
        # 특정 위험 상태(COLLAPSED이면서 움직임이 NONE 등)가 일정 시간(WARNING/CRITICAL sec) 이상 
        # 지속될 때에만 위급도를 점진적으로 승격(CAUTION -> WARNING -> CRITICAL) 시킵니다.
